...
def main():
    ...
    train_env = Monitor(gym.make('CustomHopper-source-v0', param=0.1, distribution="normal"))
    train_env.seed(SEED)
    ...
    if policy=="PPO":
        ...
        if tuning:
            ...
            model = PPO("MlpPolicy", train_env, verbose=1, n_steps=4096, learning_rate=1e-3, clip_range=0.1, seed=SEED)
            model.learn(total_timesteps=1e6)
            ...
            print(f"Average mean: {means.mean()}, average std: {stds.mean()}")
