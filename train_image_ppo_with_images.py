...
class CombinedWrapper(gym.ObservationWrapper):
    def __init__(self, env, n_frames=4):
        ...
        self.observation_space = spaces.Dict({
            "image": spaces.Box(low=0.0, high=1.0, shape=(n_frames, 300, 130), dtype=np.float32),
...
class CombinedExtractor(BaseFeaturesExtractor):
    def __init__(self, observation_space: spaces.Dict, features_dim=512):
        ...
        self.linear = torch.nn.Sequential(
            torch.nn.Linear(n_flatten + 64, features_dim),
            torch.nn.ReLU()
        )

    def forward(self, obs):
        image_feat = self.cnn(obs["image"])
        state_feat = self.mlp(obs["state"])
...
def main():
    ...
    print_plot_rewards(rewards, "Evaluation Combined Policy (image + state)")
